...
export function TokenHeader({ token, address }: TokenHeaderProps) {
    return (
        <div className="relative">
            <div className="relative">
                <img src={`${process.env.NEXT_PUBLIC_IPFS_URL}${token.metadata.bannerUri}`} alt={token.name} className="w-full h-64 object-cover rounded-lg" />
            ...
            <div className="absolute left-4 bottom-5 md:left-5 md:bottom-10 flex md:items-end justify-between gap-5 md:gap-3 flex-col md:flex-row w-full">
                <div className="flex items-center gap-3">
                    <img src={`${process.env.NEXT_PUBLIC_IPFS_URL}${token.metadata.tokenUri}`} alt={token.name} className="w-20 h-20 rounded-xl border object-cover border-gray-100 shadow-md bg-white" />
